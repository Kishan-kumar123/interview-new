...
export const CompanyPrep = () => {
  ...
  // Filtering algorithmic coding problems linked to this company dynamically via database metadata
  const filteredCodingQuestions = codingQuestions.filter(q => {
    if (q.companies && Array.isArray(q.companies)) {
      return q.companies.includes(selectedCompany);
    }
    // Fallback if companies are not specified
