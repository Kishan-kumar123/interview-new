...
export const AdminDashboard = () => {
  ...
  const handleEndorseCertificate = async (id) => {
    try {
  ...
  const handleCreateCodingQuestion = async (e) => {
    e.preventDefault();
    setFormFeedback('');
    ...
    try {
      const payload = {
        ...
        companies: newCompanies ? newCompanies.split(',').map(c => c.trim()).filter(Boolean) : []
      };
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-slate-900/10 border border-slate-850 rounded-2xl p-6">
                ...
                <form onSubmit={handleCreateCodingQuestion} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Target Hiring Companies (Comma-separated)</label>
                    <input
                      type="text"
                      placeholder="e.g. Google, Microsoft, Amazon, Walmart"
                      value={newCompanies}
